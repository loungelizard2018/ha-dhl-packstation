...
class DHLPackstationCard extends HTMLElement {
  ...
  static getStubConfig(hass) {
    const entity = Object.keys(hass.states).find(
      (id) =>
        hass.states[id].attributes?.data_type ===
  ...
  _weekly() {
    ...
    return ORDER.map(
      (dayOfWeek) =>
        source.find((item) => item.dayOfWeek === dayOfWeek) || {
          dayOfWeek,
          capacity: "unknown",
        },
    );
  ...
  full() {
    ...
    return `<div class="background" style="--bg:url('${status.image}');--status:${status.color};--glow:${status.glow}">
      ...
      <section class="forecast">
        ...
        <div class="note"><ha-icon icon="mdi:information-outline"></ha-icon><span>DHL-Prognose anhand vergangener Wochen, kein aktueller Live-Füllstand</span></div>
      </section>
    </div>`;
  }
...
class DHLPackstationCardEditor extends HTMLElement {
  ...
  render() {
    ...
    this.innerHTML = `<style>.editor{display:grid;gap:12px;padding:16px}label{display:grid;gap:5px}select,input{width:100%;box-sizing:border-box;padding:10px;border-radius:8px}</style>
      <div class="editor">
        <label>Entität<select id="entity">${entities}</select></label>
        <label>Ansicht<select id="view">
          <option value="full" ${this.config.view === "full" ? "selected" : ""}>Groß</option>
          <option value="compact" ${this.config.view === "compact" ? "selected" : ""}>Kompakt</option>
          <option value="row" ${this.config.view === "row" ? "selected" : ""}>Zeile</option>
        </select></label>
        <label>Titel<input id="name" value="${this.config.name || ""}"></label>
      </div>`;
...
customElements.define(
  "dhl-packstation-card-editor",
  DHLPackstationCardEditor,
);
